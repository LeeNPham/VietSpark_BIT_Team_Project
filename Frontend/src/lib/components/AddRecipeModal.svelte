...
<Modal onclose={toggleModal}>
	<div class="mb-6 sm:mb-6 md:mb-10 lg:mb-10 space-y-4">
	...
	{#if imageOption === 'upload'}
		<div class="flex flex-col mb-10">
			<label for="recipe-image" class="text-sm text-secondary-green mb-2">Upload Recipe Image</label>
			<input id="recipe-image" type="file" accept="image/*" onchange={handleImageChange}  class="text-sm text-secondary-forest"/>
			{#if newImageFile}
				<p class="text-sm text-secondary-forest">Selected: {newImageFile.name}</p>
				<button onclick={uploadImage} class="mt-2 rounded-lg bg-secondary-green px-4 py-1 text-white"
	...
	{#if imageOption === 'generate'}
		<div class="mb-10 space-y-4">
			<p class="text-sm sm:text-sm md:text-lg lg:text-lg text-secondary-forest">Generate an image based on the recipe name.</p>
	...
	<div class="mb-10 space-y-4">
		...
		<div>
			{#each ingredientRows as ingredient, index}
				<div class="items-center flex flex-row gap-2 sm:gap-2 md:gap-4 lg:gap-4 mb-3">
					<Input
						type="text"
					...
					/>
					<button onclick={() => removeIngredientRow(index)} class={customStyles.recipeRemove}>Remove</button>
				</div>
			{/each}
		</div>
	</div>
	...
	<div class="mb-10 flex items-center gap-4">
		...
		<div class="space-y-2">
			...
			<Input id="servings" type="text" bind:value={newServings} placeholder="Enter serving"  class={customStyles.inputBox}/>
		</div>
		<div class="space-y-2">
			<label for="calories" class={customStyles.recipeh2}>Calories</label>
			<Input id="calories" type="text" bind:value={newCalories} placeholder="Enter calories" class={customStyles.inputBox}/>
		</div>
	</div>
		...
		<div class="space-y-3">
			{#each newInstructions as instruction, index}
				<div class="flex items-center gap-2">
					...
					/>
					<button onclick={() => removeInstruction(index)} class={customStyles.recipeRemove}>Remove</button>
				</div>
			{/each}
		</div>
	</div>
	<div>
		<button
			...
			class="text-xs sm:text-sm md:text-lg lg:text-lg rounded-2xl bg-secondary-forest p-2 text-white outline outline-secondary-green w-full">Submit</button>
	</div>
	</Modal>
{/if}
