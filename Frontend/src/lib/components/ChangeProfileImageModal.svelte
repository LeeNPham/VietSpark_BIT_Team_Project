	...
	<div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
		<div class="w-80 rounded-lg bg-white p-6 shadow-lg">
			...
			{#if profileImagePreview}
				<div class="mt-4">
					<img src={profileImagePreview} alt="" class="h-auto w-full rounded-lg" />
				</div>
			{/if}
			<div class="mt-4 flex justify-between">
				<button
					class="bg-primary-gray outline-secondary-green rounded-2xl text-lg text-gray-800 outline p-2"
					on:click={toggleProfileImageModal}>Cancel</button
				>
				<button
					class="bg-primary-orange outline-secondary-green rounded-2xl text-lg text-gray-800 outline p-2"
					on:click={uploadProfileImage}>Upload</button
				>
			</div>
		</div>
	</div>
